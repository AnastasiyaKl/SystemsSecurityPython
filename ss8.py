...
def check_user(username, passwd):
    with open("data/users8.json", "r") as file:
        data = json.load(file)
        file.close()
...
def add_new_user(username, passwd):
    ...
    with open("data/users8.json", "w") as file:
...
def decipher():
    ...
    f.close()
    obj = AES.new(sessionkey, AES.MODE_ECB)
    bplaintext = obj.decrypt(ciphertext)
    bplaintext = unpad(bplaintext, BLOCK_SIZE)
    f = open('data/users8.json', 'wb')
    f.write(bplaintext)
    f.close()
    # print('end decipher()')
...
def about():
    ...
    tk.Label(about_screen, text="Variant #1").pack()
